...
GLvoid Keyboard::KillWindow()
{
	...
	if(hrc)
	{
		...
		{
			MessageBox(0, 0, "SHUTDOWN ERROR", MB_OK|MB_ICONINFORMATION);
		}
...
GLboolean Keyboard::CreateWindow_(wchar_t *title,GLint width,GLint height,GLint bits,GLboolean fullscreenflag)
{
	...
	if(!RegisterClass(wc))
	{
		MessageBox(0, 0, "ERROR",MB_OK|MB_ICONEXCLAMATION);
		return false;
	}
	...
	if(fullscreen)
	{
		...
		{
			if(MessageBox(0, 0, "HINT",MB_YESNO|MB_ICONEXCLAMATION)==IDYES)
				fullscreen=false;
			else
			{
				MessageBox(0, 0, "ERROR",MB_OK|MB_ICONSTOP);
				return false;
			}
		}
	}
